...
class ObjectParser {
    ...
    static parseContent(content) {
        const tokenizer = new Tokenizer_1.Tokenizer(content);
    ...
    parseObject() {
        ...
        switch (token.type) {
            case Tokenizer_1.TokenType.KEYWORD:
                this.position++;
            ...
            case Tokenizer_1.TokenType.NUMBER:
                ...
                if (this.position + 1 < this.tokens.length &&
                    this.tokens[this.position].type === Tokenizer_1.TokenType.NUMBER &&
                    this.tokens[this.position + 1].type === Tokenizer_1.TokenType.OPERATOR &&
            ...
            case Tokenizer_1.TokenType.HEX_STRING:
                this.position++;
                return { type: 'string', value: this.hexToString(token.value) };
            case Tokenizer_1.TokenType.NAME:
                this.position++;
                return { type: 'name', value: token.value };
            case Tokenizer_1.TokenType.ARRAY_START:
                return this.parseArray();
            case Tokenizer_1.TokenType.DICT_START:
    ...
    parseArray() {
        ...
        while (this.position < this.tokens.length) {
            const token = this.tokens[this.position];
            if (token.type === Tokenizer_1.TokenType.ARRAY_END) {
                this.position++;
    ...
    parseDictionary() {
        ...
        while (this.position < this.tokens.length) {
            const token = this.tokens[this.position];
            if (token.type === Tokenizer_1.TokenType.DICT_END) {
                this.position++;
                // Check if followed by stream
                if (this.position < this.tokens.length &&
                    this.tokens[this.position].type === Tokenizer_1.TokenType.STREAM) {
                    return this.handleStreamDictionary(dict);
                }
                return dict;
            }
            if (token.type === Tokenizer_1.TokenType.NAME) {
    ...
    handleStreamDictionary(dict) {
        ...
        while (this.position < this.tokens.length) {
            const token = this.tokens[this.position];
            if (token.type === Tokenizer_1.TokenType.ENDSTREAM) {
                this.position++;
                return {
                    type: 'stream',
                    dictionary: dict,
